...
def main():
    ...
    # ===== Загрузка champion-модели из MLflow =====
    print(f"\n>>> Loading CatBoost model from MLflow: {MODEL_URI}")
    model = mlflow.catboost.load_model(MODEL_URI)
    if not isinstance(model, CatBoostClassifier):
        raise RuntimeError(f"Loaded model is not CatBoostClassifier: {type(model)}")
    print("✅ Model loaded:", type(model))

    # ======== 1. Базовое тестирование на тест-сете ========
    print("\n>>> Base evaluation on test set (champion: CatBoost Balanced)")
    ...
    # --- Scenario 1: High amount x3 ---
    print("\n>>> Stress test scenario: High amount x3")
    ...
    # --- Scenario 2: All transactions at night ---
    print("\n>>> Stress test scenario: All transactions at night")
    ...
    # --- Scenario 3: High burstiness x3 ---
    print("\n>>> Stress test scenario: High burstiness x3")
    ...
    # ======== 3. SHAP для champion-модели ========
    print("\n>>> Computing SHAP for champion model (CatBoost Balanced)")
