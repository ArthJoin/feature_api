# train_balanced_and_ensemble.py
...
# ======= здесь важно: подставь правильный модуль с функциями =======
...
def evaluate_binary_model(
    ...
    """
    Сканируем пороги, ищем лучший по F_beta.
    Строим ROC/PR графики и сохраняем.
...
def plot_global_roc_pr(models_info, y_true, proba_dict):
    """
    Общий ROC и PR-графики для нескольких моделей.
    models_info: список имён ["CB_BALANCED", "XGB_NUMERIC", "ENSEMBLE"]
...
def compute_shap_catboost(model_cb, X_sample: pd.DataFrame, prefix: str):
    """
    SHAP-графики для CatBoost Balanced.
    """
    ...
    plt.close()
    print(f"Saved CatBoost SHAP bar to: {path_bar}")
    ...
    plt.close()
    print(f"Saved CatBoost SHAP beeswarm to: {path_bee}")


def compute_shap_xgb(model_xgb, X_sample: pd.DataFrame, prefix: str):
    """
    SHAP-графики для XGBoost numeric.
    """
    ...
    plt.savefig(path_bar, bbox_inches="tight")
    plt.close()
    print(f"Saved XGB SHAP bar to: {path_bar}")
    ...
    plt.savefig(path_bee, bbox_inches="tight")
    plt.close()
    print(f"Saved XGB SHAP beeswarm to: {path_bee}")
...
def main():
    ...
    # Статистики + фичеринг
    print("\n>>> Считаю train-статистики ...")
    stats = compute_feature_stats(df_train_raw)

    print(">>> Строю фичи для train/test ...")
    ...
    print(
        f"\nВсего фичей после feature engineering: {len(all_cols)} "
    ...
    print(f"\n>>> [Model 1] CatBoost Balanced (no SMOTE, class weights)")
    ...
    # 2) XGBoost Numeric
    # =======================
    print("\n>>> [Model 2] XGBoost на числовых фичах + scale_pos_weight")
    ...
    # =======================
    # 4) SHAP для CatBoost и XGB
    # =======================
    # возьмём небольшой сэмпл теста для SHAP, чтобы не умереть по времени
    ...
    # =======================
    # 5) Выбор чемпиона и логирование в MLflow
    # =======================
    # Для хакатона берём чемпионом именно CB_BALANCED
    champion = metrics_cb
    print("\n=== Champion model: CatBoost Balanced ===")
    print(champion)
    ...
    with mlflow.start_run(run_name="CB_BALANCED_final") as run:
        ...
        # логируем основные параметры CatBoost
        mlflow.log_params(params_cb_balanced)

        # логируем метрики чемпиона
        ...
        print("\n>>> Регистрирую модель в Model Registry...")
        ...
        try:
            ...
            print(
                f"\n✅ Модель CatBoost Balanced зарегистрирована как "
...
if __name__ == "__main__":
    # Для корректных SHAP-плотов иногда нужно:
    shap.initjs()
    main()
